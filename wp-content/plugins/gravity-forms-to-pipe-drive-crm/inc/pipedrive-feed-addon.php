...
if (class_exists("GFForms") && !class_exists('WPGravityFormsToPipedriveAddOn')) {
    ...
    class WPGravityFormsToPipedriveAddOn extends GFFeedAddOn {
		...
		public function plugin_page_icon(){
			...
			return $this->_wpgf2pdcrm_plugin_folder_url.'images/help-for-wordpress-small.png"';
		}
		...
		public function plugin_settings(){
			...
			if( $token_saved ){
				...
				echo '<p>                        
						...
						<input type="button" id="wpgf2pdcrm_refresh_pipedrive_data_cache_button_id" class="button-primary" value="Refresh Pipedrive data cache" style="margin-left:20px;"/>
                        <span id="wpgf2pdcrm_refresh_pipedrive_data_cache_ajax_loader_id" style="display: none;">
							<img src="'.plugin_dir_url("").'gravity-forms-to-pipe-drive-crm/images/ajax-loader.gif" />
						</span>
					  </p>';
		...
		public function plugin_settings_icon(){
			return '<img src="'.$this->_wpgf2pdcrm_plugin_folder_url.'images/help-for-wordpress-small.png" alt="Help For WordPress Logo" style="float:left;" />';
		}
		...
		public function form_settings( $form ) {
			...
			if ( ! $pipedrive_token ){
				$addon_setting_page = add_query_arg( array( 'page' => 'gf_settings', 'subview' => 'pipedirveaddon' ),  admin_url( 'admin.php' ) );
                ...
                array(
                    ...
                    "fields" => array(
						...
						array(
							...
							'default_value'	=> '',
                            "tooltip" => "<h6>Feed behaviour</h6>Choose to create a contact / a deal / an activity.",
                            "class"   => "medium"
                        )
					 )
				),
        ...
        public function process_feed($feed, $entry, $form){
			...
			if( $new_person && 
				...
				if( $checking_for_duplicates && count($email_array) > 0 ){
					$this->wpgf2pdcrm_api_CLASS->wpgf2pdcrm_debug_push( 'check_for_duplicates', 'Duplicates checking enabled!' );
					
					$duplicate_person_id = $this->wpgf2pdcrm_api_CLASS->wpgf2pdcrm_get_person_by_email( $token_saved, $email_array[0] );
				}
		...
		public function settings_field_map( $field, $echo = true ) {
			...
			$html .= '<table class="settings-field-map-table wpgf2pdcrm-settings-filed-map-header" cellspacing="0" cellpadding="0" style="width:95%">' .
						'<thead>
			...
			$html .= '<table class="settings-field-map-table wpgf2pdcrm-settings-filed-map" cellspacing="0" cellpadding="0" style="width:100%;">
						 <tbody>';
		...
		public function settings_select_deal_title( $field, $echo = true ) {
			...
			$html .= sprintf( '<span class="wpgf2pdcrm-deal-title-selector-container"><select %1$s>%2$s</select>', implode( ' ', $attributes ), $this->get_select_options( $field['choices'], '' ) );
			$html .= ' OR </span><span class="wpgf2pdcrm-deal-title-custom-text-container"><input type="text" placeholder="Custom text..." class="gaddon-setting gaddon-select wpgf2pd-deal-title-values-add-custom-text-input" id="wpgf2pdcrm_deal_title_custom_text_'.$random_id.'" rel="'.$random_id.'" />';
			...
			$html .= '<input type="hidden" name="_gaddon_setting_' . $name . '" value="'.$value.'" id="wpgf2pdcrm_deal_title_val_'.$random_id.'" />';
		...
		public function settings_select_deal_person( $field, $echo = true ) {
			...
			$html .= '<input type="hidden" name="_gaddon_setting_' . $name . '" value="'.$value.'" id="wpgf2pdcrm_deal_person_val_'.$random_id.'" />';
			$html .= rgar( $field, 'after_select' );
		...
		public function settings_select_deal_org( $field, $echo = true ) {
			...
			$html .= '<input type="hidden" name="_gaddon_setting_' . $name . '" value="'.$value.'" id="wpgf2pdcrm_deal_org_val_'.$random_id.'" />';
			$html .= rgar( $field, 'after_select' );
		...
		public function settings_select_deal_note( $field, $echo = true ) {
			...
			foreach( $saved_deal_title_field_array as $field_val ){
				...
				$deal_title_fields_span_str .= '<span style="margin-right:10px;"><a class="ntdelbutton wpgf2pd-deal-notes-values-list-del-icon" rel="'.$field_val.'" relselectorid="'.$random_id.'">X</a>&nbsp;'.$field_label.'</span>';
			}
            ...
            $html .= '<label style="margin-left:10px;"><input type="checkbox" name="_gaddon_setting_' . $name . '_gf2zoho_all_in_one" value="YES"'.$all_in_one_checked.'/> Combine all fields into one note</label>';
            
			$html .= '<input type="hidden" name="_gaddon_setting_' . $name . '" value="'.$value.'" id="wpgf2pdcrm_deal_notes_val_'.$random_id.'" />';
			$html .= rgar( $field, 'after_select' );
		...
		public function settings_select_deal_product( $field, $echo = true ) {
			...
			$html .= '<input type="hidden" name="_gaddon_setting_' . $name . '" value="'.$value.'" id="wpgf2pdcrm_deal_product_val_'.$random_id.'" />';
			$html .= rgar( $field, 'after_select' );
		...
		public function settings_select_activity_type( $field, $echo = true ) {
			...
			$html .= '</select>';
			$html .= '<input type="hidden" name="_gaddon_setting_' . $name . '" value="'.$value.'" id="wpgf2pdcrm_activity_type_val_'.$random_id.'" />';
			$html .= rgar( $field, 'after_select' );
