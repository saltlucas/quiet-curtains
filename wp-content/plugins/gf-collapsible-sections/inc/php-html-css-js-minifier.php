...
if (!class_exists('PHP_HTML_CSS_JS_Minifier')) {
	class PHP_HTML_CSS_JS_Minifier {

		protected $MINIFY_STRING = '"(?:[^"\\\]|\\\.)*"|\'(?:[^\'\\\]|\\\.)*\'';
		protected $MINIFY_COMMENT_CSS = '/\*[\s\S]*?\*/';
		protected $MINIFY_COMMENT_HTML = '<!\-{2}[\s\S]*?\-{2}>';
		protected $MINIFY_COMMENT_JS = '//[^\n]*';
		protected $MINIFY_PATTERN_JS = '\b/[^\n]+?/[gimuy]*\b';
		protected $MINIFY_HTML = '<[!/]?[a-zA-Z\d:.-]+[\s\S]*?>';
		protected $MINIFY_HTML_ENT = '&(?:[a-zA-Z\d]+|\#\d+|\#x[a-fA-F\d]+);';
		protected $MINIFY_HTML_KEEP = '<pre(?:\s[^<>]*?)?>[\s\S]*?</pre>|<code(?:\s[^<>]*?)?>[\s\S]*?</code>|<script(?:\s[^<>]*?)?>[\s\S]*?</script>|<style(?:\s[^<>]*?)?>[\s\S]*?</style>|<textarea(?:\s[^<>]*?)?>[\s\S]*?</textarea>';
		// escape character
		protected $X = "\x1A";
		...
		public static function get_instance() {
			if ( self::$_instance == null ) {
				self::$_instance = new PHP_HTML_CSS_JS_Minifier();
			}
		...
		public function minify_css($input, $comment = 2, $quote = 2) {
			return $this->fn_minify_css($input, $comment, $quote);
		}

		public function minify_html($input, $comment = 2, $quote = 1) {
			return $this->fn_minify_html($input, $comment, $quote);
		}

		public function minify_js($input, $comment = 2, $quote = 2) {
			return $this->fn_minify_js($input, $comment, $quote);
		}
